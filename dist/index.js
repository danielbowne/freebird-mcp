...
function validateAndSanitizeUrl(urlString) {
    ...
    // Block access to private networks and sensitive endpoints
    ...
    // Additional check for IPv6 private ranges
    if (hostname.match(/^f[cd][0-9a-f]{2}:/i)) {
        throw new Error('Access to private IPv6 networks is not allowed');
    }
        ...
        tools: [
            {
                ...
                description: '🕊️ Smart web search with DuckDuckGo. Just add "use freebird" to your prompt.',
                inputSchema: {
                    type: "object",
                    properties: {
                        query: {
            ...
            {
                name: "freebird_fetch",
                description: '🦅 Fetch and extract full page content from URLs. Returns LLM-friendly Markdown with preserved structure, code blocks, and tables.',
                inputSchema: {
                    type: "object",
                    properties: {
                        url: {
